function [mosaic,tform_data] = video_sweep_calibration(frames,nth_frame)
...
featWinLen   = 25;            % Length of feature window 
...
% current and the previous video frames. 
 hcornerdet = vision.CornerDetector( ...
    ...
    'IntensityThreshold', 0.05, 'MaximumCornerCount', maxNumPoints, ...
...
hVideo1.Position(2) = hVideo1.Position(2) - 900; 
hVideo1.Position([3 4]) = [800 800];
...
%     roi = int32([2 2 size(I, 2)-2 size(I, 1)-2]);
    roi = int32([0 0 size(I, 2) size(I, 1)]);
